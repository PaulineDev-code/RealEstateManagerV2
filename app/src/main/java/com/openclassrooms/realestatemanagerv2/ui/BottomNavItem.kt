...
sealed class BottomNavItem(val route: String, val title: String, val icon: ImageVector) {
    object List : BottomNavItem("home_screen", "List", Icons.AutoMirrored.Filled.List) {
        const val ARG_NEW_ID :String = "newId"

        val fullRoute :String = "$route?$ARG_NEW_ID={$ARG_NEW_ID}"

        fun routeWith(newId: String?): String =
            if (newId.isNullOrEmpty()) route
            else "$route?$ARG_NEW_ID=${Uri.encode(newId)}"
    }
